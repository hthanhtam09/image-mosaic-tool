...
export default function MosaicWorkspace({
  ...
  const { gateNotice, setGateNotice, requestPaidAccess } =
    useAccessGate(access);
    ...
    fontFace
      ...
      `;
        ...
        window.dispatchEvent(
          new CustomEvent("custom-font-loaded", { detail: customFontName }),
        );
      })
      .catch((err) => {
  ...
  const objectFocusStepParam =
    activeTab === "object-focus" ? searchParams.get("step") : null;
  ...
  const handleGridTypeChange = useCallback(
    (gridType: ColorByNumberGridType | "auto") => {
      if (gridType === "auto") {
        const cycle = ALL_PATTERNS.filter(
          (p) =>
            p !== "square-mark" &&
            p !== "hexagon-mark" &&
            enabledPatterns[p] !== false,
        );
    ...
    onImportSuccess: useCallback(
      (mode: "standard" | "object-focus") => {
        if (mode === "object-focus") {
          ...
          if (folderName) {
            router.replace(
              `/studio/projects/${toSlug(folderName)}/object-focus?step=convert`,
            );
          }
          return;
        }
  ...
  const {
    isConverting: importIsConverting,
    setIsConverting,
    importProgress,
    isProcessingFolder,
    keepImportScreen,
  ...
  const {
    ...
    handleDownloadAllImages: handleDownloadObjectFocusImages,
  } = objectZipHook;
  ...
  const handleConvertAll = useCallback(async () => {
    ...
    const idleCount = projects.filter(
      (p) => p.status === "idle" || p.status === "error",
    ).length;
    if (idleCount > access.maxConvertAtOnce) {
      requestPaidAccess(
        `Your ${access.plan} workspace can convert ${access.maxConvertAtOnce} image(s) at once.`,
      );
      return;
    }
  ...
  const handleConvertSelected = useCallback(async () => {
    if (isConverting) return;
    const toConvert = projects.filter(
      (p) =>
        selectedIds.has(p.id) && (p.status === "idle" || p.status === "error"),
    );
    ...
    setIsConverting(true);
    try {
      const convertSingleProject =
        useColorByNumberStore.getState().convertSingleProject;
      const cpuCores =
  ...
  const handleConvertObjectFocus = useCallback(
    async (onlySelected: boolean) => {
      ...
      try {
        ...
        await Promise.all(
          Array.from(
            { length: Math.min(limit, toConvert.length) },
            runProcessor,
          ),
        );
      } finally {
  ...
  const shouldShowImportScreen =
    ...
    (activeTab !== null &&
      ...
      activeTab !== "object-focus" &&
      !(activeTab === "before-after" && beforeAfterJob)) ||
  ...
  const selectedIdleCount = projects.filter(
    (p) =>
      selectedIds.has(p.id) && (p.status === "idle" || p.status === "error"),
  ).length;
  const objectFocusIdleCount = objectFocusProjects.filter(
    (p) => p.status === "idle" || p.status === "error",
  ).length;
  const selectedObjectFocusIdleCount = objectFocusProjects.filter(
    (p) =>
      selectedIds.has(p.id) && (p.status === "idle" || p.status === "error"),
  ).length;
  ...
  return (
    <div id="workspace" className="h-full flex overflow-hidden">
          ...
          <div className="relative z-10 flex h-full w-full items-center justify-center">
            <EmptyState
              ...
              onTabSelect={(tab) => {
                ...
                if (projectName) {
                  router.replace(
                    `/studio/projects/${toSlug(projectName)}/${tab}`,
                  );
                }
              }}
              ...
              handleImageFileChange={(e) =>
                handleImageFileChange(e, projects.length)
              }
              transparentImageInputRef={transparentImageInputRef}
              handleTransparentImageFileChange={(e) =>
                handleTransparentImageFileChange(
                  e,
                  projects.length,
                  setCurrentStep,
        ...
        <ObjectFocusWorkspace
          ...
          onSelectAll={() =>
            setSelectedIds((prev) => {
              const next = new Set(prev);
              const allSelected =
                objectFocusProjects.length > 0 &&
                objectFocusProjects.every((project) => next.has(project.id));
              if (allSelected) {
                objectFocusProjects.forEach((project) =>
          ...
          onDeleteSelected={() => {
            ...
            if (
              !window.confirm(
                `Delete ${objectFocusSelectedIds.size} selected object image(s)? This cannot be undone.`,
              )
            )
              return;
          ...
          onBackToImport={() => {
            ...
            if (folderName)
              router.replace(
                `/studio/projects/${toSlug(folderName)}/object-focus?step=import`,
              );
          }}
        />
      ) : (
        <div className="min-w-0 flex-1 flex flex-col p-8 overflow-hidden relative">
          {gateNotice && (
            <div className="mb-4 flex items-start justify-between gap-4 rounded-lg border border-[var(--accent)]/30 bg-[var(--accent)]/10 px-4 py-3 text-sm text-(--text-primary)">
              ...
              <div className="flex shrink-0 items-center gap-2">
                <a
                  href={gateNotice.href}
                  className="rounded-md bg-[var(--accent)] px-3 py-1.5 text-xs font-semibold text-[var(--bg-primary)]"
                >
                  {gateNotice.action}
                </a>
                <button
                  ...
                  className="rounded-md px-2 py-1 text-xs text-(--text-secondary) hover:bg-white/10 hover:text-(--text-primary)"
                >
                  Dismiss
                </button>
              </div>
            ...
            <div className="flex items-center gap-3.5 min-w-0">
              {currentStep === 3 ? (
                ...
                >
                  ...
                  <span className="text-sm font-medium pr-0.5 select-none">
                    Back to PDF Setup
                  </span>
                </button>
              ) : currentStep === 2 ? (
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (projectName) {
                      router.replace(
                        `/studio/projects/${toSlug(projectName)}/image-import`,
                      );
                    }
                  }}
                  disabled={isAnyActionRunning}
              ...
              {currentStep === 1 && projects.length > 0 && (
                <div
                  className="flex items-center gap-2 pl-4 ml-1.5 border-l border-white/10"
                  ...
                  {currentStep === 1 &&
                    ...
                    !isConverting && (
                      <div className="flex gap-3 items-center">
                        ...
                        {directImages.some(
                          (img) => img.colorUrl && img.uncolorUrl,
                        ) && (
                          <button
          ...
          <StepContainer isActive={currentStep === "design-config"}>
            <BookDesignConfigStep
              ...
              firstImageUrl={
                projects[0]?.thumbnailDataUrl ||
                directImages[0]?.colorUrl ||
      ...
      <input
        ...
        className="hidden"
        multiple
        onChange={(e) => handleImageFileChange(e, projects.length)}
      />
      <input
        ...
        multiple
        onChange={(e) =>
          handleTransparentImageFileChange(e, projects.length, setCurrentStep)
        }
      />
      <input
        ref={dirInputRef}
      ...
      <DownloadProgressModal
        isOpen={
          isGeneratingPdf || isZipping || isObjectFocusZipping || baIsZipping
        }
        type={
          isGeneratingPdf
            ? "pdf"
      ...
      <div className="flex h-full min-h-0 flex-col">
        <div className="shrink-0 border-b border-[var(--border-default)] pb-5">
          ...
          <p className="mt-1 max-w-2xl text-sm text-[var(--text-secondary)]">
            This mode uses mark patterns and removes white backgrounds during
            conversion.
          </p>
        </div>
            ...
            {isImporting ? (
              ...
              >
                ...
                <p className="text-base font-semibold text-[var(--text-primary)]">
                  Select object images
                </p>
                <p className="mt-1 text-sm text-[var(--text-secondary)]">
                  PNG or JPG files with white backgrounds work best.
                </p>
              </button>
            )}
        ...
        <div className="shrink-0 border-b border-[var(--border-default)] pb-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 max-w-2xl text-sm text-[var(--text-secondary)]">
                White backgrounds are removed during conversion and exports use
                transparent artwork without palette columns.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {isZipping
                  ? `Preparing ${zipProgress.current}/${zipProgress.total}`
                  : "Download PNGs"}
              </button>
            </div>
          </div>
                ...
                return (
                  <article
                    ...
                    className={`overflow-hidden rounded-xl border bg-[var(--bg-secondary)] transition ${
                      isSelected
                        ? "border-purple-400/70 shadow-lg shadow-purple-950/20"
                        : "border-[var(--border-default)] hover:border-purple-400/35"
                    }`}
                  >
                    <button
                      type="button"
                    ...
                    <div className="space-y-3 p-3">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateProject(project.id, {
                              gridType: event.target
                                .value as ColorByNumberGridType,
                              status: "idle",
                            })
                          }
                      ...
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            void useColorByNumberStore
                              .getState()
                              .convertSingleProject(project.id)
                          }
                          disabled={
                            isAnyActionRunning || isProcessing || isReady
                          }

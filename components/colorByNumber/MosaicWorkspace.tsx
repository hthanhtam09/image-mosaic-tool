...
export default function MosaicWorkspace({
  ...
  const [cachedDesignConfigKey, setCachedDesignConfigKey] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    const basePath = activeTab ? `/studio/projects/${slug}/${activeTab}` : `/studio/projects/${slug}`
    ...
    (gridType: ColorByNumberGridType | 'auto') => {
      if (gridType === 'auto') {
        const cycle = ALL_PATTERNS.filter(
          (p) => p !== 'square-mark' && p !== 'hexagon-mark' && enabledPatterns[p] !== false
        )
  ...
  const isAnyActionRunning =
    isConverting || isPreparingStep2 || isGeneratingPdf || isZipping || isObjectFocusZipping || baIsZipping
  ...
  const shouldShowImportScreen =
    ...
    (activeTab === null &&
      ...
      ((projects.length === 0 && !isFolderModeActive && !beforeAfterJob) || (isFolderModeActive && currentStep === 1)))

  const idleCount = projects.filter((p) => p.status === 'idle').length
  const selectedIdleCount = projects.filter(
    (p) => selectedIds.has(p.id) && (p.status === 'idle' || p.status === 'error')
  ).length
  ...
  return (
    <div id="workspace" className="h-full flex overflow-hidden">
        ...
        <ObjectFocusWorkspace
          ...
          onSelectAll={() =>
            setSelectedIds((prev) => {
              const next = new Set(prev)
              const allSelected =
                objectFocusProjects.length > 0 && objectFocusProjects.every((project) => next.has(project.id))
              if (allSelected) {
          ...
          onDeleteSelected={() => {
            ...
            if (
              !window.confirm(`Delete ${objectFocusSelectedIds.size} selected object image(s)? This cannot be undone.`)
            )
              return
          ...
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-[var(--border-default)]">
            ...
            {currentStep === 1 && (
              <div className="flex gap-5 items-center">
                ...
                {((selectedIds.size > 0 ? selectedIdleCount > 0 : idleCount > 0) ||
                  isAnyActionRunning ||
                  (currentStep === 1 &&
                    (projects.length > 0 || directImages.length > 0) &&
                    (selectedIds.size > 0 ? selectedIdleCount === 0 : idleCount === 0) &&
                    !isAnyActionRunning)) && <div className="h-6 w-px bg-[var(--border-default)] mx-3 self-center" />}
                  ...
                  {currentStep === 1 &&
                    ...
                    !isConverting && (
                      <div className="flex gap-3 items-center">
                        {projects.length > 0 &&
                          (globalGridType === 'square-mark' || globalGridType === 'hexagon-mark') && (
                            <button
      ...
      <input
        ...
        className="hidden"
        multiple
        onChange={(e) => handleTransparentImageFileChange(e, projects.length, setCurrentStep)}
      />
      <input
        ref={dirInputRef}
        type="file"
      ...
      <DownloadProgressModal
        ...
        type={
          isGeneratingPdf ? 'pdf' : isZipping || isObjectFocusZipping ? 'zip' : baIsZipping ? 'before-after-zip' : null
        }
        progress={
          isGeneratingPdf
            ? pdfProgress
        ...
        <div className="shrink-0 border-b border-[var(--border-default)] pb-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 max-w-2xl text-sm text-[var(--text-secondary)]">
                White backgrounds are removed during conversion and exports use transparent artwork without palette
                columns.
              </p>
            </div>

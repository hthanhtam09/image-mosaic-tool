...
description: >
  ...
  code cleaner and more maintainable. Use this skill before committing a feature, after a
  refactor, or whenever you suspect code bloat in any TypeScript/TSX file or feature module.
---
...
This is **image-mosaic-tool** — a Next.js 16 / React 19 app with:

- **Tailwind v4** — config is done via `@theme { ... }` in `app/globals.css`, NOT `tailwind.config.ts`. Custom design tokens are CSS variables (`--color-bg-primary`, `--color-accent`, etc.) exposed as Tailwind utilities (`bg-bg-primary`, `text-accent`, etc.).
- **No shadcn/ui** — no `@/components/ui/*`, no `DataTable`, no `Skeleton` component library.
- **No icon library** — no `lucide-react`, no `@heroicons`. Icons are inline SVGs or CSS-driven.
- **Zustand** for client state (`store/` directory).
- **Supabase** for auth/data (`utils/supabase/`, `lib/featureFlags*.ts`).
- **Folder structure**:
  - `app/` — Next.js App Router pages/layouts/API routes
  - `components/` — UI components grouped by concern (`colorByNumber/`, `admin/`, `marketing/`, `shared/`, `tools/`)
  - `lib/` — pure utility/processing logic (image processing, palette, quantize)
  - `store/` — Zustand stores
  - `utils/` — thin infrastructure helpers (Supabase client setup)
...
7. **Duplicate / merge candidates** — two or more files that share the same concern and can be safely merged.
8. **Code style & readability** — small patterns that can be simplified without changing behaviour.
9. **Tailwind rule compliance** — replace ad-hoc arbitrary values with project design tokens/utilities.
10. **Shared component reuse** — replace duplicate UI blocks with existing shared/feature components.
11. **Oversized files / split candidates** — files that have grown too long should be broken into smaller files by responsibility.
12. **Loading UX** — replace plain-text "Loading…" placeholders with inline skeleton UI (no external skeleton library — build inline with Tailwind `animate-pulse` blocks).
...
- When a file grows beyond ~200 lines and feels "busy", or approaches/exceeds **~250 lines** — treat as a **split candidate**.
- When TypeScript reports `no-unused-vars` or ESLint warns about dead code.
- When you're asked to "clean up", "optimize", or "slim down" a file or feature.
...
| Specific sub-folder    | e.g. `lib/`, `store/`, `components/`     |
...
Apply the checks below in order. For each issue found, record:

- File path (relative to workspace root).
- Line number(s).
- Issue category (see table).
- Recommended action.
...
**Action**: Remove the unused import line (or the specific binding if it is a named import).

Special case — **type-only imports**: if a type is only used in a type position and the project uses `verbatimModuleSyntax`, keep `import type { ... }` and remove regular imports.
...
const PAGE_SIZE = 20
const unusedMapper = (x: Foo) => x.id
```

**Action**: Delete the declaration. If the value is needed elsewhere, move it to a shared constants file and export.
...
**Action**: Delete. If the type is genuinely shared, move it to the nearest `types.ts` or inline in the consuming file.
...
3. It is not an entry point (route file, `layout.tsx`, `page.tsx`, `route.ts`).
...
This project uses **Tailwind v4** with a custom `@theme` block in `app/globals.css`. All design tokens are available as named utilities. Do not use arbitrary bracket values for anything covered by the theme.

**Available design tokens → Tailwind utilities:**
...
| `--color-accent-hover`  | `text-accent-hover`    |
| `--color-accent-deep`   | `text-accent-deep`     |
...
**Action**: Replace text/emoji pseudo-icons with inline SVGs consistent with existing icon patterns in the codebase.
...
### Tailwind Violations
- `components/marketing/home/Hero.tsx` line 42: `text-[#ffffff]` → **REPLACE** with `text-text-primary`

### Split Candidates
- `components/colorByNumber/Dashboard.tsx` (~350 lines) → **SPLIT** into `components/colorByNumber/dashboard/DashboardHeader.tsx`, `hooks/useDashboardData.ts`
  Reason: Mixes data fetching, header UI, and grid layout.

### Loading (text-only)
- `components/colorByNumber/Dashboard.tsx` line 88: `{isLoading ? <p>Loading…</p> : ...}` → **REPLACE** with `animate-pulse` skeleton blocks.
```
...
7. **Execute splits** for oversized files (new files per split plan → thin orchestrator in original path → update imports). Do this **before** cosmetic refactors so line counts reflect real structure.
8. Apply style improvements.
9. Fix Tailwind arbitrary-value violations using project design token utilities.
10. Replace duplicated components with existing reusable components.
11. Add missing design tokens to `app/globals.css` `@theme` block and migrate usage to the new named utility.
12. Replace text/emoji pseudo-icons with inline SVGs matching existing codebase patterns.
13. Replace text-only loading UI with `animate-pulse` skeleton blocks; add `aria-busy` / `aria-label` where appropriate.
...
- After deleting, merging, or splitting files, scan all import paths in the project and remove stale references.
- Run a final grep to confirm no broken import paths remain.
- For Tailwind fixes, verify resulting utilities are named token utilities (no arbitrary fallback unless exception is documented).
- For loading fixes, verify no screen uses text-only loading for primary content.
...
4. Confirm all newly introduced UI is mapped to existing reusable components where available.
5. Confirm typography/spacing/color classes avoid bracket syntax.
6. Confirm icon rendering uses inline SVGs and not text/emoji symbols.
7. After splits, confirm no circular imports, each new file has a single clear responsibility, and orchestrator/page files stay within the **~200–250 line** guideline.
8. Confirm loading states use `animate-pulse` skeleton blocks, not text-only placeholders.

---

## Grep Patterns (Quick Reference)

Use `grep` with these patterns to find references:
...
| Arbitrary Tailwind values            | `\[#[0-9a-fA-F]`                                  |
| Text/emoji icons                     | `[✅✕→←↑↓⚠️❌🔥]`                              |
...
11. **If a needed Tailwind utility does not exist, define it in `app/globals.css` `@theme` block** and use the new named class — do not use bracket arbitrary utility in feature code.
...
14. **Never use text-only loading** for primary page content — use `animate-pulse` skeleton blocks so layout is preserved.
...
- [ ] No invalid/avoidable Tailwind arbitrary utilities remain.
